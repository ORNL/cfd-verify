...
Verification, validation, and uncertainty quantification (VVUQ) is a set of activities which add credibility to modeling and simulation results. Broadly, verification is the activity of making sure the problem is solved right, validation is the activity of making sure the model represents reality, and uncertainty quantification is the activity of accounting for the natural uncertainties of systems. Often, the activities of verification and validation (V&V) are discussed separately from uncertainty quantification (UQ). Within V&V, there are actually three activities, (1) code verification, (2) solution verification, and (3) validation. First, code verification ensures that the equations implemented in a solver are error free. Next, solution verification quantifies the error in a particular solution of a model. Finally, validation compares simulation results to experiments to quantify the model's error relative to the real world. 
